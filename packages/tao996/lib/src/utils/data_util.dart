...
class MyDataUtil {
  ...
  static DateTime? getDateTime(dynamic v, {DateTime? defaultValue}) {
    return MyDatetimeUtil.parse(v) ?? defaultValue;
  }
  ...
  /// 将任意对象转为 JSON 字符串。
  static String jsonString(dynamic data) => const JsonEncoder().convert(data);

  /// 深拷贝（通过 JSON 序列化/反序列化）。
  static dynamic copy(dynamic data) => jsonDecode(jsonEncode(data));

  /// 校验字符串是否匹配正则。
  static bool hasMatch(String data, String pattern) =>
      RegExp(pattern).hasMatch(data);
